...
def parse_versioned_dir(path: Path) -> dict[str, Any]:
    """Parse {project}_{stage}_{date}_v{version} source artifact directory names."""
...
def file_entry(base_dir: Path, relative_path: str, *, role: str,
...
def correction_item_refs(items: list[Any]) -> list[dict[str, Any]]:
    refs: list[dict[str, Any]] = []
    for item in items:
        refs.append({
            "item_id": getattr(item, "item_id", ""),
            "conflict_type": getattr(item, "conflict_type", ""),
            "target_stage": getattr(item, "target_stage", ""),
            "affected_systems": list(getattr(item, "affected_systems", []) or []),
            "affected_files": list(getattr(item, "affected_files", []) or []),
        })
    return refs
...
def write_artifact_manifest(
    ...
    *,
    ...
    manifest: dict[str, Any] = {
        ...
        "artifact_dir": meta.get("name") or output_dir.name,
        "mode": mode,
