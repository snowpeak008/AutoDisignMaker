...
Migrated from src/core/plugin_manager.py.
Change: reads PLUGIN_MANIFEST_FILE from core.paths (pipeline/_registry.json)
instead of src/plugins/plugin_manifest.json.
"""
...
from core.paths import PLUGIN_MANIFEST_FILE, PROJECT_ROOT
...
class PluginManager:
    ...
    def load_stage(self, stage_id: str) -> StagePlugin:
        ...
        if not matches:
            raise KeyError(f"Stage plugin not registered: {stage_id}")
        spec = matches[0]
        ...
        if plugin.stage_id != stage_id:
            raise ValueError(
                f"Plugin stage_id mismatch: manifest={stage_id}, class={plugin.stage_id}"
            )
...
__all__ = ["PluginSpec", "PluginManager"]
