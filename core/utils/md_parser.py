...
Migrated from tools/md_parser.py.
Change: yaml_compat import updated to core.utils.
tools.output_validator import will be updated in sub-plan 09.
"""
...
class MarkdownParser:
    ...
    def parse(self) -> dict:
        ...
        def flush_section():
            ...
            if subsection_list:
                ...
                result[section_name] = {}
            section_kv = {}; section_list = []; section_has_kv = False; section_has_list = False
            subsection_list = []; current_item = None; in_table = False
        ...
        for line in self.text.split("\n"):
            ...
            if _H3_RE.match(line):
                ...
                if m:
                    current_item = {"_name": m.group(1).strip()}
                else:
                    section_kv[heading] = {}
            ...
            if stripped.startswith("|") and stripped.endswith("|"):
                ...
                if in_table and table_headers:
                    row = {table_headers[i]: c for i, c in enumerate(cells) if i < len(table_headers)}
                    table_rows.append(row)
                elif not in_table:
                    table_headers = cells; in_table = True
                continue
            ...
            if m:
                ...
                if current_item is not None:
                    ...
                    section_kv[key] = val; section_has_kv = True
                continue
            m = _LIST_RE.match(line)
            if m:
                val = m.group(2).strip()
                if current_item is not None:
                    current_item.setdefault("_items", []).append(val)
                else:
                    section_list.append(val); section_has_list = True
                continue
    ...
    def extract_json_block(self) -> str | None:
        m = _JSON_FENCE_RE.search(self.text)
        return m.group(1).strip() if m else None

    def extract_yaml_block(self) -> str | None:
        m = _YAML_FENCE_RE.search(self.text)
        return m.group(1).strip() if m else None
    ...
    if last_block:
        ...
        if json_text and json_text.strip() != str(text).strip():
            try:
                ...
                parsed = {}
        if parsed:
            _check_types_and_keys(parsed, allowed_types, required_keys, output_name)
            return parsed
...
def _check_types_and_keys(parsed: Any, allowed_types: tuple, required_keys: Iterable[str] | None, output_name: str) -> None:
