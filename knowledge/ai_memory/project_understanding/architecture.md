...
AutoDesignMaker 是一个**Step00-17 确定性游戏设计文档流水线**。用户在 GUI 中完成设计决策后，导出为 `.md`/`.json` 文件，放入当前 draft 的 `source_artifacts/`，流水线将其转换为可执行的 Unity C# 代码和美术资产需求。
...
| 层 | 目录 | 核心职责 | 可修改性 |
|----|------|----------|----------|
| **步骤插件层** | `pipeline/` | 每个阶段的业务逻辑，22个插件（D1-D4 + step_00~17） | 主要修改区 |
...
| **注册表层** | `pipeline/artifact_layer/` | registry.json 声明 Step00-17 制品依赖关系 | 随步骤更新 |
...
DesignEngine.export_project()  写入 .md/.json 到 sandbox/source_artifacts/
  ↓
用户运行：python -m core.main --auto-approve --from-step 0 --stop-step 17
  ↓
...
- `_stage7_art_style_generation_outputs()` / `_stage8_art_style_confirmation_outputs()` — 美术风格生成与人工确认门禁
- `_parse_design_text()` — 解析设计文档的通用解析器
- `_apply_unity_package_changes()` — 修改 Unity manifest.json 的依赖项

**特点**：
- 单一巨文件（5370行），Step00-17 逻辑混合
...
**pipeline/artifact_layer/registry.json**：声明 Step00-17 的制品 bundle，每个 bundle 包含：
- `stage`: int
...
stage_07/style_options.json
  ↓ (步骤08 美术风格确认)
stage_08/style_confirmation.json
  ↓ (步骤09/10 程序计划与美术计划)
stage_09/program_task_breakdown.json
stage_10/art_task_breakdown.json
  ↓ (步骤12 程序执行) ← 调用 AI 生成 Unity C# 代码
stage_12/execution_objects.json
  ↓ (步骤14 集成验证)
stage_14/integration_test_report.json
  ↓ (步骤15 构建打包)
stage_15/build_output/
  ↓ (存档)
