...
AutoDesignMaker 是一个**16阶段确定性游戏设计文档流水线**。用户在 GUI 中完成设计决策后，导出为 `.md`/`.json` 文件，放入 `sandbox/source_artifacts/`，流水线将其转换为可执行的 Unity C# 代码和美术资产需求。
...
| 层 | 目录 | 核心职责 | 可修改性 |
|----|------|----------|----------|
| **步骤插件层** | `pipeline/` | 每个阶段的业务逻辑，20个插件（D1-D4 + step_00~15） | 主要修改区 |
...
| **注册表层** | `artifact_layer/` | registry.json 声明16阶段制品依赖关系 | 随步骤更新 |
...
DesignEngine.export_project()  写入 .md/.json 到 sandbox/source_artifacts/
  ↓
用户运行：python -m core.main --auto-approve --from-step 0 --stop-step 15
  ↓
...
@dataclass
class StageResult:
    status: StageStatus  # "success" | "failed" | "skipped" | "blocked"
    outputs: dict[str, Any]
    errors: list[str]
    warnings: list[str]
```
...
**artifact_layer/registry.json**：声明16个阶段的制品 bundle，每个 bundle 包含：
- `stage`: int
...
stage_03/program_requirements.json
  ↓ (步骤07 程序计划)
stage_07/program_task_breakdown.json
  ↓ (步骤10 程序执行) ← 调用 AI 生成 Unity C# 代码
stage_10/execution_objects.json
  ↓ (步骤12 集成验证)
stage_12/integration_test_report.json
  ↓ (步骤13 构建打包)
stage_13/build_output/
  ↓ (存档)
...
**为什么 generation.py 有3795行？**  
历史原因。16个阶段最初都在一个文件里快速迭代，后续未拆分。每个 `_stageN_outputs()` 是独立逻辑块，互不调用。
