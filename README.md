...
- `Codex CLI` / `Claude Code CLI`：选择本地 CLI 类型，确保已安装并登录。
- AI 配置按 `开发API` / `生图API` / `补全API` 三类分别启用。

底部状态栏会显示当前开发 API、DevFlow 进度和系统运行状态；点击进度可切换到开发流水线并定位到当前未完成步骤。
...
- `settings/ai_config.json`：本地统一 AI 配置（v3 三分类），包含 API Key，已忽略，禁止提交。
